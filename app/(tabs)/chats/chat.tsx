...
export default function Chat() {
  ...
  const initializeChat = async () => {
    try {
      ...
      // sanitize incoming param (handles undefined or the literal string "undefined")
      ...
      if (!sanitizedInfluencerId) {
        console.error(
          "Missing or invalid influencerId param:",
          rawInfluencerId
        );
      ...
      if (user.id === sanitizedInfluencerId) {
        console.warn("User attempted to open chat with themselves:", user.id);
        Alert.alert("Error", "You cannot chat with yourself.");
        router.back();
        return;
      }

      // Try to fetch existing conversation ONLY if we have a valid UUID string
      let { data: existingConversation, error: convError } = await supabase
        .from("conversations")
        .select("*")
        .eq("regular_user_id", user.id)
        .eq("influencer_id", sanitizedInfluencerId)
        .single();

      // It's fine if conversation doesn't exist; log other errors
      if (convError && convError.code !== "PGRST116") {
        console.error("Error fetching conversation:", convError);
      }

      // If no conversation exists, create one (safe insert)
      if (!existingConversation) {
        const { data: newConversation, error: createError } = await supabase
          .from("conversations")
          .insert({
            regular_user_id: user.id,
            influencer_id: sanitizedInfluencerId,
            is_active: true,
          })
          .select()
          .single();
      ...
      await loadMessages(existingConversation.id, user.id);
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      {/* Input Area - Rest of the code remains the same */}
      <View className="bg-black px-4 pb-20 pt-3">
        <View className="flex-row items-end">
          <TouchableOpacity
            className="mr-3 mb-2"
