...
export default function Chat() {
  const params = useLocalSearchParams(); // expects params passed from Discover navigation
  const partnerName = (params?.name as string) ?? "Bam Margera";
  const partnerImage =
    (params?.image as string) ??
    "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=100";
  // if you passed conversationId from Discover, it'll be available here:
  const routeConversationId = (params?.conversation_id as string) ?? null;
  const partnerIsOnline = (params?.isOnline as string) === "true";

  const [message, setMessage] = useState("");
  const [balance, setBalance] = useState(12); // local balance (you should use real wallet)
  const [messages, setMessages] = useState<Message[]>([]);
  const [contextMenu, setContextMenu] = useState<Message | null>(null);
  const [editingMessage, setEditingMessage] = useState<Message | null>(null);
  const [replyTo, setReplyTo] = useState<Message | null>(null);
  ...
  const [partnerId, setPartnerId] = useState<string | null>(null); // optional: if Discover passes partner user_id
  const [loading, setLoading] = useState(false);
  const realtimeSubRef = useRef<any>(null);
  const [recipientPricePerMessage, setRecipientPricePerMessage] =
    useState<number>(0); // fetch from influencer_profiles

  // Sample gallery (kept from your original)
  const galleryImages = [
    "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400",
    "https://images.unsplash.com/photo-1551817958-fa1f5d8d26d2?w=400",
    "https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=400",
  ...
  // --- Helper: calculate cost (keeps same formula you had) ---
  const calculateCost = (text: string) => {
    const length = text.length;
    if (length === 0) return 0;
    if (length <= 60) return 0.05;
  ...
  // --- Helper: determine conversation (best-effort) ---
  const getOrCreateConversation = useCallback(
    async (
      ...
      // If conversationId was provided via route, trust that first
      if (providedConversationId) {
        return providedConversationId;
      }

      if (!otherUserId) {
        // can't find partner id — return null; UI will still work but messages can't be saved
        console.warn("getOrCreateConversation: otherUserId missing");
        return null;
      }

      // Try multiple strategies to find an existing conversation:
      // 1) participant_ids array contains both IDs (common pattern)
      try {
        const { data: convByArray, error: err1 } = await supabase
          .from("conversations")
          .select("id")
          .contains("participant_ids", [authUserId, otherUserId])
          .limit(1);

        if (!err1 && convByArray?.length) return convByArray[0].id;
      } catch (e) {
        // ignore — maybe column doesn't exist
      }

      // 2) columns like user1/user2 or participant_one/participant_two
      const pairsToTry = [
        { a: "user1", b: "user2" },
        { a: "participant_one", b: "participant_two" },
      ];
      ...
      for (const pair of pairsToTry) {
        try {
          const { data } = await supabase
            ...
            .select("id")
            .or(
              `${pair.a}.eq.${authUserId},${pair.b}.eq.${otherUserId}`,
              // also check reversed pair
              `${pair.a}.eq.${otherUserId},${pair.b}.eq.${authUserId}`
            )
            .limit(1);
      ...
      // 3) If not found, attempt to create a conversation row. This insertion payload may need to
      // be adapted to your conversations schema (change `participant_ids` or `user1/user2` accordingly).
      try {
        const payload: any = {};
        // prefer participant_ids array pattern
        payload.participant_ids = [authUserId, otherUserId];
        ...
        if (insertErr) {
          console.error(
            "create conversation error (tried participant_ids):",
            insertErr
          );
          // try alternative insert with user1/user2 fields (some schemas use those)
          ...
          if (insertErr2) {
            console.error(
              "create conversation error (tried user1/user2):",
              insertErr2
            );
            return null;
          }
        ...
        console.error("getOrCreateConversation fatal error:", e);
        return null;
      }
    },
    []
  ...
  // --- Load current user and initialize conversation/messages ---
  // --- Load current user and initialize conversation/messages ---
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        ...
        // Prefer explicit partner_id param; fallback to other possible names
        const routePartnerId =
          (params?.partner_id as string) ||
          (params?.partnerId as string) ||
        ...
        // If route provided a conversation id, trust it
        if (routeConversationId) {
          setConversationId(routeConversationId);
        }

        // Try to fetch recipient's price_per_message (if partnerId known)
        const effectivePartnerId = routePartnerId ?? params?.partner_id;
        if (effectivePartnerId) {
          try {
            const { data: profileData } = await supabase
              ...
              .single();
            setRecipientPricePerMessage(
              Number(profileData?.price_per_message) || 0
            );
          } catch (e) {
            setRecipientPricePerMessage(0);
          }
        }

        // Resolve conversation id only if we don't already have one
        if (!routeConversationId) {
          // only attempt to create/find a conversation when we have a partner id
          if (routePartnerId) {
            ...
            if (convId) {
              ...
              console.warn(
                "No conversation id resolved/created. Messages won't be persisted until conversation exists."
              );
            }
          } else {
            console.warn(
              "No partner id in route params — conversation not created automatically."
            );
          }
        } else {
          // we have routeConversationId
          await loadMessages(routeConversationId);
          subscribeToMessages(routeConversationId);
        }
      } catch (err) {
    ...
    // cleanup on unmount: remove realtime
    return () => {
  ...
  const loadMessages = useCallback(
    async (convId: string) => {
      ...
      const mapped = (data || []).map((m) => ({
        ...m,
        amount_charged: m.amount_charged ? Number(m.amount_charged) : null,
  ...
  // --- Realtime subscription to messages for this conversation ---
          ...
          (payload) => {
            ...
            const msg: Message = {
              ...
              amount_charged: record.amount_charged
                ? Number(record.amount_charged)
                : null,
              isSent: record.sender_id === currentUserId,
            };
  ...
  const markMessageDelivered = async (messageId: string) => {
    try {
      ...
      console.error("markMessageDelivered error:", e);
    }
  };

  // --- Mark all messages in conv read (call when opening the chat) ---
  const markAllRead = async (convId: string) => {
    try {
      await supabase
        .from("messages")
  ...
  // --- Send message (persists to messages table if conversationId available) ---
  // --- Send message (persists to messages table if conversationId available) ---
  const handleSendMessage = async () => {
    if (!message.trim()) return;

    const computedCost =
      recipientPricePerMessage > 0
        ? recipientPricePerMessage
        : calculateCost(message);

    if (computedCost > 0 && balance < computedCost) {
    ...
    // Use conversationId if present; otherwise leave undefined on temp message for UI
    const tempMsg: Message = {
      id: tempId,
      conversation_id: conversationId ?? routeConversationId ?? "",
      sender_id: currentUserId ?? "",
      content: message,
      content_type: "text",
      amount_charged: computedCost > 0 ? computedCost : null,
      is_paid: computedCost > 0,
      isSent: true,
      created_at: new Date().toISOString(),
    };
    ...
    if (computedCost > 0) {
      setBalance((b) => Number((b - computedCost).toFixed(2)));
    }

    try {
      // Ensure we have a conversation id before inserting into DB
      let convId = conversationId ?? routeConversationId ?? null;

      if (!convId) {
        // only attempt to create a conversation if we know partnerId
        if (!partnerId) {
          // rollback optimistic UI & balance and inform user
          ...
          Alert.alert(
            "Unable to send",
            "Conversation not found and partner is unknown. Open the chat from the user profile or pass a partner_id when navigating."
          );
          return;
        }

        // try to create/find conversation (this will handle participant_ids/user1-user2 patterns)
        const auth = await supabase.auth.getUser();
        const authUserId = auth?.data?.user?.id;
        if (!authUserId) {
          // rollback optimistic UI & balance
          setMessages((prev) => prev.filter((m) => m.id !== tempId));
          if (computedCost > 0) {
            setBalance((b) => Number((b + computedCost).toFixed(2)));
          }
          Alert.alert("Not authenticated", "Please sign in to send messages.");
          return;
        }

        convId = await getOrCreateConversation(authUserId, partnerId, null);
        if (!convId) {
          // rollback optimistic UI & balance
          setMessages((prev) => prev.filter((m) => m.id !== tempId));
          if (computedCost > 0) {
          ...
          Alert.alert(
            ...
            "Couldn't create a conversation. Try again later."
          );
          return;
        }

        // update local conversationId and subscribe
        setConversationId(convId);
        await loadMessages(convId);
        subscribeToMessages(convId);
      }

      // At this point convId is guaranteed non-null
      const insertPayload = {
        ...
        transaction_id: null,
        amount_charged:
          computedCost > 0 ? Number(computedCost.toFixed(2)) : null,
        is_delivered: false,
        is_read: false,
        delivered_at: null,
        read_at: null,
      ...
      Alert.alert(
        "Send failed",
        "Could not send message. See logs for details."
      );
    } finally {
      setMessage("");
      setCharCount(0);
  ...
  // --- helper UI actions (reply/copy/edit/delete/report) remain unchanged from your original ---
  const handleTextChange = (text: string) => {
    setMessage(text);
    setCharCount(text.length);
  };
  ...
  const handleEdit = () => {
    if (contextMenu) {
      setMessage(contextMenu.content);
      setEditingMessage(contextMenu);
      setContextMenu(null);
      textInputRef.current?.focus();
    }
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: async () => {
            // try to delete in DB if persisted
            if (!msgToDelete) return;
            if (!msgToDelete.id.startsWith("temp-")) {
              try {
                await supabase
                  .from("messages")
                  .delete()
                ...
                setMessages((prev) =>
                  prev.filter((m) => m.id !== msgToDelete.id)
                );
              } catch (e) {
                console.error("delete message error:", e);
                Alert.alert(
                  "Delete failed",
                  "Couldn't delete message from server."
                );
              }
            } else {
              setMessages((prev) =>
  ...
  // --- UI rendering (largely your original UI) ---
  return (
    <KeyboardAvoidingView
      className="flex-1 bg-black"
      behavior={Platform.OS === "ios" ? "padding" : undefined}
        ...
        <View className="flex-row items-center flex-1">
          ...
          <View className="flex-1">
            ...
            <View className="flex-row items-center">
              ...
              >
                {partnerIsOnline ? "Active Now" : "Last seen recently"}
              </Text>
              <View className="w-2 h-2 rounded-full bg-green-500 ml-2" />
            </View>
          </View>
          ...
          >
            <View className="flex-1 justify-center items-center px-6">
              ...
              >
                ...
                >
                  Each message costs according to recipient's price or
                ...
                >
                  {msg.replyTo && (
                    ...
                    >
                      ...
                      >
                        {msg.replyTo.isSent ? "You" : partnerName}
                      </Text>
                      <Text
                        numberOfLines={1}
                        ellipsizeMode="tail"
            ...
            {replyTo && (
              ...
              >
                ...
                >
                  ...
                  >
                    Reply to {replyTo.isSent ? "You" : partnerName}
                  </Text>
